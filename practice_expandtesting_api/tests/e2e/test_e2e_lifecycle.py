...
def test_e2e_a_happy_path_lifecycle(manage_context_primary_user_and_note_id_template):
    try:
        ...
        def step_1():
            resp = requests.get(f"{API_HEALTH_CHECK_ENDPOINT}", headers=headers_default)
        ...
        def step_2():
            ...
            resp = requests.post(f"{API_USER_REGISTER_ENDPOINT}", json=user_payload, headers=headers_default)
            check.equal(resp.status_code, 201, msg=f"Expected 201, but got {resp.status_code}")
            check.is_true(resp.json().get("success"))
            manage_context_primary_user_and_note_id_template["user_test_step"] = UserTestStep.USER_REGISTRATION
        run_step("E2E_A>Step#2. User Registration", step_2)

        def step_3():
            user_payload = context["user_payload"]
            payload = {"email": user_payload["email"], "password": user_payload["password"]}
            resp = requests.post(f"{API_USER_LOGIN_ENDPOINT}", json=payload, headers=context["headers_default"])
            check.equal(resp.status_code, 200, msg=f"Expected 200, but got {resp.status_code}")
            data = resp.json()
            ...
            manage_context_primary_user_and_note_id_template["user_test_step"] = UserTestStep.USER_LOGIN
        ...
        def step_4():
            ...
            resp = requests.post(f"{API_NOTES_ENDPOINT}", json=payload, headers=headers)
        ...
        def step_5():
            headers = {**context["headers_login"], "x-auth-token": context["token"]}
            resp = requests.get(f"{API_NOTES_ENDPOINT}/{context['note_id'][0]}", headers=headers)
            check.equal(resp.status_code, 200, msg=f"Expected 200, but got {resp.status_code}")
        ...
        def step_6():
            headers = {**context["headers_login"], "x-auth-token": context["token"]}
            resp = requests.delete(f"{API_NOTES_ENDPOINT}/{context['note_id'][0]}", headers=headers)
            check.equal(resp.status_code, 200, msg=f"Expected 200, but got {resp.status_code}")
            get_res = requests.get(f"{API_NOTES_ENDPOINT}/{context['note_id'][0]}", headers=headers)
        ...
        def step_8():
            headers = {**context["headers_login"], "x-auth-token": context["token"]}
            resp = requests.get(f"{API_NOTES_ENDPOINT}", headers=headers)
            check.equal(resp.status_code, 200, msg=f"Expected 200, but got {resp.status_code}")
...
def test_e2e_b_change_password_logout_relogin(manage_context_primary_user_and_note_id_template):
    try:
        ...
        def step_2():
            user_payload = context["user_payload"]
            resp = requests.post(f"{API_USER_LOGIN_ENDPOINT}", json=user_payload, headers=context["headers_login"])
            check.equal(resp.status_code, 200, msg=f"Expected 200, but got {resp.status_code}")
            data = resp.json()
        ...
        def step_3():
            ...
            resp = requests.post(f"{API_USER_CHANGE_PASSWORD_ENDPOINT}", json=payload, headers=headers)
            check.equal(resp.status_code, 200, msg=f"Expected 200, but got {resp.status_code}")
            check.is_true(resp.json().get("success"))
        ...
        def step_4():
            headers = {**context["headers_login"], "x-auth-token": context["token"]}
            resp = requests.delete(f"{API_USER_LOGOUT_ENDPOINT}", headers=headers)
            check.equal(resp.status_code, 200, msg=f"Expected 200, but got {resp.status_code}")
            check.is_true(resp.json().get("success"))
            manage_context_primary_user_and_note_id_template["user_test_step"] = UserTestStep.USER_LOGOUT
        run_step("E2E_B>Step#4. User Logout", step_4)

        def step_5():
            # Attempt to reach a protected profile route with the old token
            headers = {**context["headers_login"], "x-auth-token": context["token"]}
            resp = requests.get(f"{API_USER_PROFILE_ENDPOINT}", headers=headers)
        ...
        def step_6():
            user_payload = context["user_payload"]
            payload = {"email": user_payload["email"], "password": user_payload["password"]}
            resp = requests.post(f"{API_USER_LOGIN_ENDPOINT}", json=payload, headers=context["headers_login"])
            check.equal(resp.status_code, 200, msg=f"Expected 200, but got {resp.status_code}")
            data = resp.json()
...
def test_e2e_c_security_check(manage_context_primary_user_and_note_id_template, manage_context_secondary_user_and_note_id_template):
    try:
        ...
        def step_1():
            ...
            resp = requests.post(f"{API_USER_REGISTER_ENDPOINT}", json=payload_primary, headers=headers_default)
        ...
        def step_2():
            check.equal(context_primary["user_test_step"], UserTestStep.USER_REGISTRATION)
            payload_primary = context_primary["user_payload"]
            resp = requests.post(f"{API_USER_REGISTER_ENDPOINT}", json=payload_primary, headers=headers_default)
        ...
        def step_3():
            ...
            resp = requests.post(f"{API_USER_LOGIN_ENDPOINT}", json=payload, headers=headers_default)
        ...
        def step_4():
            resp = requests.get(f"{API_NOTES_ENDPOINT}", headers=context_primary["headers_login"])
        ...
        def step_5():
            ...
            # primary user login
            payload_primary = context_primary["user_payload"]
            resp_login_primary = requests.post(f"{API_USER_LOGIN_ENDPOINT}", json=payload_primary, headers=headers_default)
            ...
            resp_note_primary = requests.post(API_NOTES_ENDPOINT, headers=headers_login_primary, json=payload_note)
            ...
            resp_notes_primary = requests.get(API_NOTES_ENDPOINT, headers=headers_login_primary)
            ...
            resp_login_secondary = requests.post(f"{API_USER_REGISTER_ENDPOINT}", json=payload_secondary, headers=headers_default)
            ...
            resp_login_secondary = requests.post(f"{API_USER_LOGIN_ENDPOINT}", json=payload_secondary, headers=headers_default)
            ...
            resp_notes_secondary = requests.get(API_NOTES_ENDPOINT, headers=headers_login_secondary)
